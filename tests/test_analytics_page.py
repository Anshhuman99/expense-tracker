...
def test_analytics_loads_for_logged_in_user(client):
    ...
    client.post(
        "/login", data={"email": "analytics@example.com", "password": "password123"}
    )
    ...
    assert (
        b"Category Distribution" in response.data
        or b"No expense data found" in response.data
    )
...
def test_navbar_active_states(client):
    ...
    client.post(
        "/login", data={"email": "active@example.com", "password": "password123"}
    )
    ...
    assert (
        b'class="">Analytics</a>' in profile_response.data
        or b'class="active">Analytics</a>' not in profile_response.data
    )
    ...
    assert (
        b'class="">Profile</a>' in analytics_response.data
        or b'class="active">Profile</a>' not in analytics_response.data
    )
